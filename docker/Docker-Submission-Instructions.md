...
The Dockerfile is set up to launch the `run.sh` file as the entry point. Any other entrypoint will be ignored. The `run.sh` script expects three inputs:
...
The `run.sh` calls the Python script `main.py` passing along these three input parameters to it. This `main.py` script imports the segmentation method `segment` from the `segment.py` file. 

The `segment.py` file contains the segmentation functionality to take an input image and produce a segmentation mask for the current task.

### Where should I insert my code
Please insert your model related code in the Pyhton script `segment.py` within the `segment()` function.
...
A comment indicates where to insert your dependencies. Please do not remove any existing dependencies from the `requirements.txt` file. 
...
[ ] Set up Docker on your machine. [Get started with Docker](https://docs.docker.com/get-docker/).
[ ] Download Docker template for the appropriate task from our GitHub [templates folder](templates/).
[ ] Updated the files in the Docker template following the [instructions](#editing-the-docker-templates).

### Building the Docker image

#### How to name the docker image
...
Build the docker image following these steps:
* Browse to the directory where the `Dockerfile` is located for the template is located.
...
At any point in the following steps more information related to Synapse is needed then refer to the [Docker Registry documentation page](https://help.synapse.org/docs/Synapse-Docker-Registry.2011037752.html).
...
* Type the following command to login to synapse using docker.
```bash
...
Type the following command to tag the docker image before uploading to Synapse. 
```bash
...
Type the following command to push the local Docker image that has been tagged so that it appears in your Synapse project.  
```bash
